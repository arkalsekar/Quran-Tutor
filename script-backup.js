...
reciteBtn.addEventListener("click", async () => {
    ...
    recognition.onresult = (event) => {
        ...
        resultDiv.textContent = `You recited: ${userText} 
        </hr>  
        Correct Ayah: ${correctText} 
         <hr> 
         Match: ${match.toFixed(2)}%`;

        if (event.results[0][0].hasOwnProperty('audio')) {
            // Some browsers may provide audio, but most do not.
            // We'll use MediaRecorder to record user's voice.
        }

        // Play back user's recitation
        if (window.MediaRecorder) {
            ...
            navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
                ...
                mediaRecorder.onstop = e => {
                    ...
                    userAudio.play();
                    // Optionally, add a button to replay
                };
            }).catch(err => {
